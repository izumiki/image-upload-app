...
const Avatar = ({
  ...
  const [imageWidth, setImageWidth] = useState<number>(getWidth(avatarSrc))
  const [imageHeight, setImageHeight] = useState<number>(getHeight(avatarSrc))
  const [isOpen, setIsOpen] = useState<boolean>(false)
  const side: number = avatarSide

  useEffect(() => {
    setImageWidth(getWidth(src))
    setImageHeight(getHeight(src))
  }, [src])
  ...
  const handleFile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setImageWidth(await getWidth(url))
    setImageHeight(await getHeight(url))
    URL.revokeObjectURL
